...
Rect ArmorDetector::GetRoi(const Mat &Img){
    ...
    if(RectTmp.x==0||RectTmp.y==0||
       ...
       LostCnt>=15||DetectCnt%100==0){
        ...
        int x=int(RectTmp.x-(w-RectTmp.width)*0.5f);
        int y=int(RectTmp.y-(h-RectTmp.height)*0.5f);
...
bool ArmorDetector::DetectArmor(Mat& Img, Rect RoiRect){
    ...
    split(RoiImage, Bgr);
    Mat ResultImg;
    subtract(Bgr[2], Bgr[1], ResultImg);//图像的减法操作
    ...
    for(size_t i = 0; i < ContoursBrightness.size(); i++){
        double Area = contourArea(ContoursBrightness[i]);
